...
<template>
  <div class="h-full flex flex-col bg-gray-50 border-r border-gray-200">
    ...
    <div v-if="ticketStore.isLoading" class="flex-1 flex justify-center items-center">
      <IconLoading class="text-blue-600" />
    </div>

    <div v-else-if="!ticketStore.tickets.length" class="flex-1 flex justify-center items-center p-4">
    ...
    <ul v-else class="flex-1 overflow-y-auto">
      <li
        ...
        :class="{ 'bg-blue-50': ticketStore.selectedTicketId === ticket.id }"
      >
        <div class="p-4">
